...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.hovered_outline = ImLerp(it_anim->second.hovered_outline, (hovered ? 0.06f : 0.0f), menu.anim_speed * (1.0f - ImGui::GetIO().DeltaTime));

    // Render
    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, col, false, 2.0f);
    window->DrawList->AddRect(bb.Min, bb.Max, ImColor(1.0f, 1.0f, 1.0f, menu.m_alpha * 0.06f + it_anim->second.hovered_outline), 2.0f);
...
struct checkbox_animation {
    float rect_anim, outline_anim, text_anim;
};
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.text_anim = ImLerp(it_anim->second.text_anim, *v ? 1.0f : 0.4f, menu.anim_speed * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.rect_anim = ImLerp(it_anim->second.rect_anim, *v ? 1.0f : (hovered ? 0.2f : 0.0f), menu.anim_speed * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.outline_anim = ImLerp(it_anim->second.outline_anim, *v ? 0.06f : (hovered ? 0.06f : 0.0f), menu.anim_speed * (1.0f - ImGui::GetIO().DeltaTime));

    window->DrawList->AddRectFilled(total_bb.Min, total_bb.Min + ImVec2(square_sz, square_sz), ImColor(16 / 255.0f, 16 / 255.0f, 16 / 255.0f, menu.m_alpha), 2.0f);
    window->DrawList->AddRectFilled(total_bb.Min, total_bb.Min + ImVec2(square_sz, square_sz), ImColor(141 / 255.0f, 168 / 255.0f, 241 / 255.0f, menu.m_alpha * it_anim->second.rect_anim), 2.0f);
    RenderCheckMark(window->DrawList, ImVec2(total_bb.Min.x + 3, (total_bb.Min.y + total_bb.Max.y) / 2 - 4), ImColor(1.0f, 1.0f, 1.0f, menu.m_alpha * it_anim->second.rect_anim), 8.0f);

    window->DrawList->AddRect(total_bb.Min, total_bb.Min + ImVec2(square_sz, square_sz), ImColor(1.0f, 1.0f, 1.0f, menu.m_alpha * 0.06f + it_anim->second.outline_anim), 2.0f);
    window->DrawList->AddText(ImVec2(total_bb.Max.x - label_size.x - 5, total_bb.Min.y - 1), ImColor(1.0f, 1.0f, 1.0f, menu.m_alpha * it_anim->second.text_anim), label);
...
struct combo_animation {
    float open_anim, arrow_anim, text_anim;
};
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const float size = GetWindowWidth() - 28;

    const ImRect rect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(size, 49));
    const ImRect clickable(window->DC.CursorPos + ImVec2(0, 21), window->DC.CursorPos + ImVec2(size, 46));

    const ImVec2 arrow_size = menu.combo_arrow->CalcTextSizeA(9.0f, FLT_MAX, 0.0f, "A");
    ...
    static std::map <ImGuiID, combo_animation> anim;
    auto it_anim = anim.find(id);
    if (it_anim == anim.end())
    {
        anim.insert({ id, { 0.0f, 0.0f, 0.0f } });
    ...
    it_anim->second.arrow_anim = ImLerp(it_anim->second.arrow_anim, popup_open ? 0.3f : 0.0f, menu.anim_speed * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_anim = ImLerp(it_anim->second.text_anim, popup_open ? 1.0f : 0.4f, menu.anim_speed * (1.0f - ImGui::GetIO().DeltaTime));

    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, ImColor(33 / 255.0f, 33 / 255.0f, 33 / 255.0f, menu.m_alpha), 3.0f);
    window->DrawList->AddRect(clickable.Min, clickable.Max, ImColor(1.0f, 1.0f, 1.0f, menu.m_alpha * 0.03f), 3.0f);

    window->DrawList->AddText(rect.Min, GetColorU32(ImGuiCol_Text, it_anim->second.text_anim), label);

    RenderTextClipped(clickable.Min + ImVec2(8, 5), clickable.Max - ImVec2(24, -47), preview_value, NULL, NULL, ImVec2(0.0f, 0.0f));

    PushFont(menu.combo_arrow); {
        window->DrawList->AddText(ImVec2(clickable.Max.x - 20, (clickable.Min.y + clickable.Max.y) / 2 - arrow_size.y / 2 - 1), ImColor(1.0f, 1.0f, 1.0f, menu.m_alpha * 0.2f + it_anim->second.arrow_anim), "A");
    } PopFont();
    ...
    PushStyleColor(ImGuiCol_PopupBg, ImVec4(ImColor(33, 33, 33)));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const float w = GetWindowWidth() - 28;

    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + 12));
    const ImRect frame_bb(total_bb.Min + ImVec2(0, label_size.y + 6), total_bb.Max);
    ...
    static std::map <ImGuiID, slider_animation> anim;
    auto it_anim = anim.find(id);
    if (it_anim == anim.end())
    {
        anim.insert({ id, { 0.0f } });
    ...
    it_anim->second.opacity = ImLerp(it_anim->second.opacity, IsItemActive() ? 1.0f : 0.4f, menu.anim_speed * (1.0f - ImGui::GetIO().DeltaTime));
    ...
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(16 / 255.0f, 16 / 255.0f, 16 / 255.0f, menu.m_alpha), 5.0f);
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(grab_bb.Min.x + 3, frame_bb.Max.y), ImColor(141 / 255.0f, 168 / 255.0f, 241 / 255.0f, menu.m_alpha), 5.0f);
...
struct input_animation {
    float hover_outline, text_anim;
};
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImRect total_bb(frame_bb.Min, frame_bb.Min + total_size + ImVec2(0, 18));
    ...
    static std::map <ImGuiID, input_animation> anim;
    auto it_anim = anim.find(id);
    if (it_anim == anim.end()) {
    ...
    it_anim->second.hover_outline = ImLerp(it_anim->second.hover_outline, (IsItemActive() ? 0.06f : 0.0f), menu.anim_speed * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_anim = ImLerp(it_anim->second.text_anim, (IsItemActive() ? 1.0f : 0.4f), menu.anim_speed * (1.0f - ImGui::GetIO().DeltaTime));

    // Render frame
    if (!is_multiline)
    {
        RenderNavHighlight(frame_bb, id);
        RenderFrame(frame_bb.Min, frame_bb.Max, ImColor(16 / 255.0f, 16 / 255.0f, 16 / 255.0f, menu.m_alpha), false, 2.0f);
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(1.0f, 1.0f, 1.0f, menu.m_alpha * 0.06f + it_anim->second.hover_outline), 2.0f);
    }
    ...
    if (label_size.x > 0) {
        PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 1.0f, 1.0f, it_anim->second.text_anim)); {
            RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y - style.FramePadding.y - 15), label);
        } PopStyleColor();
    }
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    flags |= ImGuiColorEditFlags_NoTooltip | ImGuiColorEditFlags_NoInputs;
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##ColorButton", col_v4, flags, ImVec2(23, 14)))
        {
        ...
        PushStyleVar(ImGuiStyleVar_PopupBorderSize, 1.0f);
        PushStyleColor(ImGuiCol_PopupBg, ImVec4(ImColor(16, 16, 16)));
        PushStyleColor(ImGuiCol_Border, ImVec4(1.0f, 1.0f, 1.0f, 0.03f));
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    draw_list->AddCircleFilled(sv_cursor_pos, 6.f, ImColor(0, 0, 0));
    draw_list->AddCircleFilled(sv_cursor_pos, 5.f, ImColor(255, 255, 255));
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
        if (g.Style.FrameBorderSize > 0.0f)
            window->DrawList->AddRect(bb.Min, bb.Max, ImColor(0.0f, 0.0f, 0.0f, menu.m_alpha * 0.3f), rounding);
        else
            window->DrawList->AddRect(bb.Min, bb.Max, ImColor(0.0f, 0.0f, 0.0f, menu.m_alpha * 0.3f), rounding);
    }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    static std::map <ImGuiID, selectable_animation> anim;
    auto it_anim = anim.find(id);
    if (it_anim == anim.end()) {
        anim.insert({ id, { 0.0f } });
        it_anim = anim.find(id);
    ...
    it_anim->second.opacity = ImLerp(it_anim->second.opacity, (selected ? 1.0f : hovered ? 0.5f : 0.2f), menu.anim_speed * (1.0f - ImGui::GetIO().DeltaTime));
